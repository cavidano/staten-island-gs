window.onload = () => {
    ...
    var bounds = new L.GeoJSON.AJAX("./lib/geojson/statenIsland.geojson");

    bounds.on('data:loaded', function () {
        bounds.addTo(map);
        centerMap();
    });

    function centerMap(){
        map.fitBounds(bounds.getBounds(), { padding: [40, 40] });
    }
    ...
    function setMapHeight() {
        ...
        if (window.matchMedia('( min-width: 960px)').matches) {
            mapColumns.style.maxHeight = windowHeight - mapColumnsPos + "px";
            centerMap();
        } else {
            return;
        }
    ...
    window.addEventListener('resize', setMapHeight, centerMap);
